...
export function ImportAppointmentsModal({ visible, onClose, onSuccess, slug }: ImportAppointmentsModalProps) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const content = await fetchAppointmentsImportTemplate({ slug });
  ...
  const handlePreview = async () => {
    ...
    setBusy(true);
    try {
      const result = await importAppointmentsCSV(file, { dryRun: true, slug });
  ...
  const handleConfirm = async () => {
    ...
    setBusy(true);
    try {
      await importAppointmentsCSV(file, { dryRun: false, slug });
      onSuccess();
      handleClose();
    } catch (error) {
