...
export function ImportCustomersModal({ visible, onClose, onSuccess, slug }: ImportCustomersModalProps) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const content = await fetchCustomersImportTemplate({ slug });
      await saveAndShareCSV(content, 'modelo-clientes.csv');
    } catch (error) {
  ...
  const handlePreview = async () => {
    ...
    setBusy(true);
    try {
      const result = await importCustomersCSV(file, { dryRun: true, slug } as any);
  ...
  const handleConfirm = async () => {
    ...
    setBusy(true);
    try {
      await importCustomersCSV(file, { dryRun: false, slug } as any);
      onSuccess();
      handleClose();
    } catch (error) {
